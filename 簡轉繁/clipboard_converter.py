...
def main():
    print("=" * 50, flush=True)
    print("🔄 剪貼簿簡轉繁工具已啟動", flush=True)
    print("=" * 50, flush=True)
    print("• 程式會自動監控剪貼簿", flush=True)
    print("• 當偵測到簡體中文時，會自動轉換為繁體", flush=True)
    ...
    try:
            ...
            if current_content != last_content and current_content:
                ...
                if contains_simplified_chinese(current_content):
                    ...
                    if set_clipboard(converted):
                        conversion_count += 1
                        print(f"✅ 轉換成功 (第 {conversion_count} 次)", flush=True)
                        print(f"   原文: {current_content[:50]}{'...' if len(current_content) > 50 else ''}", flush=True)
                        print(f"   繁體: {converted[:50]}{'...' if len(converted) > 50 else ''}", flush=True)
                        print(flush=True)
        ...
        print(flush=True)
        print("=" * 50, flush=True)
        print(f"🛑 程式已停止，共完成 {conversion_count} 次轉換", flush=True)
        print("=" * 50, flush=True)
